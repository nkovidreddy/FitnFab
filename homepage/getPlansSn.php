<?php
    if(isset($_GET["myplan"])){
        ...
        $sqlPlan="SELECT Food.Food_Id,Food.Food_Name,Food.Calorie_Count from Account JOIN Diet_Plan on Diet_Plan.Plan_Id= Account.Plan_Id JOIN Displays on Displays.Plan_Id= Account.Plan_Id JOIN Food on Food.Food_Id=Displays.Food_Id where Displays.Type_Name='Snacks' and Diet_Plan.Plan_Id like '{$plan}' and Account.Account_Id='$username'";
        $resultPlan=mysql_query($sqlPlan);
        $plansSn = array();
        while($row = mysql_fetch_array($resultPlan)){
            array_push($plansSn, $row["Food_Name"],$row["Calorie_Count"],$row["Food_Id"]);
        }
        echo json_encode($plansSn);
